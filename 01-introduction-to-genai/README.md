...
A Transformer is a neural network architecture that processes sequences of data, such as text, by focusing on the relationships between all elements in the sequence, regardless of their position. It was a groundbreaking innovation because, unlike previous models, it doesn't process data sequentially
...
For instance, when you ask ChatGPT, **"What is the capital of France?"**, the process begins with tokenization, where the entire query is converted into a sequence of numerical IDs. The core of the model, a Transformer architecture, then processes these tokens not as isolated numbers, but as contextual embeddings, which are vectors that dynamically shift their meaning based on their relationship to other tokens in the sequence—a feat achieved by the self-attention mechanism. This allows the model to understand that the word "capital" is directly linked to "France," unlike older models. With this deep contextual understanding, the model calculates a probability distribution over its entire vocabulary to predict the most probable next token. It then selects a token—in this case, "Paris," which has the highest probability—and enters a loop, where this new token is added to the sequence, and the model predicts the next one, continuing until it generates a complete and coherent response, which is then converted back from tokens into the final text you see
